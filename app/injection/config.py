...
class JoyrideConfigSource:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.data, dict):
            raise ValueError(f"Configuration data must be a dictionary, got {type(self.data)}")
        ...
        for key, schema in self.nested_schemas.items():
            if key in config:
                if not isinstance(config[key], dict):
                    raise ValueError(f"Nested configuration '{key}' must be a dictionary")
...
class JoyrideConfigLoader:
    ...
    def load_from_environment(self, prefix: Optional[str] = None) -> JoyrideConfigSource:
    ...
    def load_from_file(self, file_path: Union[str, Path], priority: int = 50) -> JoyrideConfigSource:
...
def create_config(
    ...
    return JoyrideConfig(
        data=merged_data,
        sources=loader.sources.copy(),
        schema=schema
    )
