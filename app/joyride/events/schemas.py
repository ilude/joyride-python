...
class NodeEventSchema(EventSchema):
    ...
    node_address = StringField(
        data_key="node_address"
    )
    node_port = IntField(
        data_key="node_port"
    )
    ...
    # Cluster state
    incarnation = IntField(
        data_key="incarnation",
        default=0
    )
    cluster_size = IntField(
        data_key="cluster_size",
        default=1
    )
...
class ErrorEventSchema(EventSchema):
    ...
    retry_count = IntField(
        data_key="retry_count",
        default=0
    )
...
class HealthEventSchema(EventSchema):
    ...
    previous_status = StringField(
        data_key="previous_status",
        default="unknown"
    )
    
    # Check details
    check_duration = IntField(
        data_key="check_duration"
    )
    response_time = IntField(
        data_key="response_time",
        default=0
    )
