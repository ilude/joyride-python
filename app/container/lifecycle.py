...
class JoyrideProviderComponent(JoyrideLifecycleComponent):
    ...
    def __init__(self, name: str, provider: JoyrideProvider, registry: JoyrideProviderRegistry):
    ...
    async def stop(self) -> None:
        ...
        try:
            with self._lock:
                if self.state not in (JoyrideLifecycleState.STARTED, JoyrideLifecycleState.FAILED):
                    return  # Already stopped or stopping
                
                self.state = JoyrideLifecycleState.STOPPING
            
            # Call stop method if instance has one
            if self.instance and hasattr(self.instance, 'stop') and callable(getattr(self.instance, 'stop')):
            ...
            if self.instance and hasattr(self.instance, 'health_check'):
                try:
                    ...
                    if isinstance(status, JoyrideHealthStatus):
                        ...
                        self.health_status = JoyrideHealthStatus.HEALTHY if status else JoyrideHealthStatus.UNHEALTHY
                        ...
                        self.health_status = status_mapping.get(status.lower(), JoyrideHealthStatus.UNKNOWN)
                        return self.health_status
                
                except Exception as e:
                    logging.warning(f"Health check failed for component {self.name}: {e}")
                    self.health_status = JoyrideHealthStatus.UNHEALTHY
                    return self.health_status
...
class JoyrideLifecycleManager:
    ...
    def unregister_component(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if component.state not in (JoyrideLifecycleState.STOPPED, JoyrideLifecycleState.FAILED):
    ...
    def add_dependency(self, component_name: str, dependency_name: str) -> None:
        ...
        with self._lock:
            ...
            self._logger.info(f"Added dependency: {component_name} depends on {dependency_name}")
    
    def _has_circular_dependency(self, component_name: str, new_dependency: str) -> bool:
        ...
        with self._lock:
            ...
            def visit(name: str):
                if name in temp_visited:
                    raise JoyrideLifecycleDependencyError(f"Circular dependency detected involving {name}")
                if name in visited:
                    return
    ...
    async def start_all(self) -> None:
        ...
        for name in startup_order:
            ...
            if component.state != JoyrideLifecycleState.STOPPED:
                self._logger.warning(f"Component {name} is not in stopped state, skipping")
                continue
            ...
            try:
                ...
                self._logger.info(f"Started component: {name} (took {component.get_startup_time():.2f}s)")
            
            except asyncio.TimeoutError:
                raise JoyrideLifecycleTimeoutError(f"Component {name} startup timed out after {self.startup_timeout}s")
            except Exception as e:
    ...
    async def stop_all(self) -> None:
        ...
        for name in shutdown_order:
            ...
            if component.state not in (JoyrideLifecycleState.STARTED, JoyrideLifecycleState.FAILED):
                self._logger.warning(f"Component {name} is not in started/failed state, skipping")
                continue
            ...
            try:
                ...
                self._logger.info(f"Stopped component: {name} (took {component.get_shutdown_time():.2f}s)")
        ...
        if errors:
            raise JoyrideLifecycleError(f"Some components failed to stop: {'; '.join(errors)}")
    ...
    async def start_component(self, name: str) -> None:
        ...
        components_to_start = [
            comp_name for comp_name in components_to_start
            if self.components[comp_name].state == JoyrideLifecycleState.STOPPED
        ]
        
        self._logger.info(f"Starting component {name} and dependencies: {components_to_start}")
        ...
        for comp_name in components_to_start:
            ...
            try:
                ...
                raise JoyrideLifecycleError(f"Failed to start component {comp_name}") from e
    ...
    async def stop_component(self, name: str) -> None:
        ...
        components_to_stop = [
            comp_name for comp_name in components_to_stop
            if self.components[comp_name].state in (JoyrideLifecycleState.STARTED, JoyrideLifecycleState.FAILED)
        ]
        
        self._logger.info(f"Stopping component {name} and dependents: {components_to_stop}")
