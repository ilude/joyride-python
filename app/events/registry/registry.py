...
class JoyrideEventRegistry:
    ...
    def register_event_type(self, event_class: Type[JoyrideEvent]) -> None:
        ...
        if not issubclass(event_class, JoyrideEvent):
            raise ValueError(f"Event class {event_class} must inherit from JoyrideEvent")
        ...
        with self._lock:
            if class_name in self._event_types:
                if self._event_types[class_name] != event_class:
                    raise ValueError(f"Event type {class_name} already registered with different class")
    ...
    def get_matching_subscriptions(self, event: JoyrideEvent) -> List[JoyrideEventSubscription]:
        """
    ...
    def get_subscription_info(self, subscription_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            return {
                ...
                "handler": subscription.handler.__name__ if hasattr(subscription.handler, '__name__') else str(subscription.handler)
            }
