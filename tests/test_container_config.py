...
class TestJoyrideConfigSchema:
    ...
    def test_key_validation_type_checking(self):
        ...
        schema = JoyrideConfigSchema(
            key_types={"port": int, "host": str}
        )
        
        # Valid types
        assert schema.validate_key("port", 8080) == True
        assert schema.validate_key("host", "localhost") == True
    ...
    def test_key_validation_custom_validators(self):
        ...
        # Valid values
        assert schema.validate_key("port", 8080) == True
        assert schema.validate_key("name", "test") == True
    ...
    def test_schema_validation_required_keys(self):
        ...
        assert schema.validate(config) == True
        
        # Missing required key
        config = {"host": "localhost"}
        with pytest.raises(ValueError, match="Required configuration key 'port' is missing"):
    ...
    def test_schema_validation_nested(self):
        ...
        schema = JoyrideConfigSchema(
            ...
            nested_schemas={"service": nested_schema}
        )
        ...
        with pytest.raises(ValueError, match="Nested configuration 'service' must be a dictionary"):
            schema.validate(config)
...
class TestJoyrideConfigLoader:
    ...
    def test_add_source_validation(self):
        ...
        with pytest.raises(ValueError, match="Source must be a JoyrideConfigSource instance"):
    ...
    def test_load_from_environment(self):
        ...
        assert source.data["enabled"] == True
    ...
    def test_merge_sources(self):
        ...
        defaults = JoyrideConfigSource("defaults", 0, {
            "dns": {"port": 53, "host": "0.0.0.0"},
            "logging": {"level": "INFO"}
        }, "default")
        
        file_config = JoyrideConfigSource("file", 50, {
            "dns": {"port": 8053},  # Override port
            "docker": {"socket": "tcp://localhost:2376"}  # New key
        }, "file")
        
        env_config = JoyrideConfigSource("env", 100, {
    ...
    def test_parse_env_value_types(self):
        ...
        assert loader._parse_env_value("1") == True
        assert loader._parse_env_value("0") == False
...
class TestCreateConfig:
    ...
    def test_create_config_with_schema(self):
        ...
        schema = JoyrideConfigSchema(
            required_keys=["dns"],
            optional_keys=["docker", "swim", "hosts", "logging", "events"],  # Allow other default keys
            nested_schemas={
                "dns": JoyrideConfigSchema(
                    required_keys=["port"],
                    optional_keys=["host", "ttl", "backend"],  # Allow other DNS default keys
