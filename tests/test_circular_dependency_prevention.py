...
class TestCircularDependencyPrevention:
    ...
    def test_dns_sync_manager_local_only_parameter(self):
        ...
        for i in range(5):
            sync_manager.add_dns_record(
                f"test{i}.example.com", f"1.2.3.{i}", local_only=True
            )
        assert callback_tracker.calls["dns_callback"] == 0
    ...
    def test_dns_record_callback_prevents_infinite_loop(self):
        ...
        with patch.object(dns_server, "add_record") as mock_dns_add, patch.object(
            dns_server, "remove_record"
        ) as mock_dns_remove:
            if dns_sync_manager:
                with patch.object(
                    dns_sync_manager, "add_dns_record"
                ) as mock_sync_add, patch.object(
                    dns_sync_manager, "remove_dns_record"
                ) as mock_sync_remove, patch.object(
                    dns_sync_manager, "running", True
                ):
                    # Test add operation
                    ...
                    mock_sync_add.assert_called_once_with(
                        "test.example.com", "1.2.3.4", local_only=True
                    )
                    ...
                    # Verify DNS sync manager was called with local_only=True
                    mock_sync_remove.assert_called_once_with(
                        "test.example.com", local_only=True
                    )
    ...
    def test_integration_scenario_no_circular_dependency(self):
        ...
        def tracked_dns_callback(action: str, hostname: str, ip_address: str):
            ...
            callback_tracker.track_call(
                "dns_record_callback", action, hostname, ip_address
            )
            ...
            if action == "add":
                ...
                callback_tracker.track_call(
                    "dns_sync_add_local_only", hostname, ip_address
                )
            elif action == "remove":
                callback_tracker.track_call(
                    "dns_sync_remove_local_only", hostname, ip_address
                )
        ...
        sync_manager.add_dns_record(
            "joyride.example.com", "172.17.0.2", local_only=True
        )
    ...
    def test_concurrent_dns_operations_no_deadlock(self):
        ...
        def worker(thread_id: int):
            ...
            for i in range(5):
                # Normal operations
                sync_manager.add_dns_record(
                    f"test{thread_id}-{i}.example.com", f"10.0.{thread_id}.{i}"
                )

                # Local-only operations (should not trigger callbacks)
                sync_manager.add_dns_record(
                    f"local{thread_id}-{i}.example.com",
                    f"10.1.{thread_id}.{i}",
                    local_only=True,
                )
    ...
    def test_edge_cases_preventing_regression(self):
        ...
        sync_manager = DNSSyncManager(
            node_id="test-edge-node", dns_callback=Mock(), host_ip="127.0.0.1"
        )
        ...
        sync_manager_no_callback = DNSSyncManager(
            node_id="test-no-callback", dns_callback=None, host_ip="127.0.0.1"
        )

        # Should not crash with None callback
        sync_manager_no_callback.add_dns_record("test.example.com", "1.2.3.4")
        sync_manager_no_callback.add_dns_record(
            "test.example.com", "1.2.3.4", local_only=True
        )
    ...
    def test_dns_sync_manager_callback_isolation(self):
        ...
        sync_manager = DNSSyncManager(
            node_id="test-isolation", dns_callback=primary_callback, host_ip="127.0.0.1"
        )
    ...
    def test_circular_dependency_fix_backward_compatibility(self):
        ...
        sync_manager = DNSSyncManager(
            node_id="test-compat", dns_callback=callback, host_ip="127.0.0.1"
        )
