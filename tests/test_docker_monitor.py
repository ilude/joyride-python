...
class TestDockerEventMonitor:
    ...
    def monitor(self, dns_callback):
        ...
        return DockerEventMonitor(dns_callback, "192.168.1.100")
    ...
    def mock_container_with_label(self):
        ...
        container = Mock()
        container.attrs = {
            "Config": {"Labels": {"joyride.host.name": "test.example.com"}}
        }
        return container
    ...
    def test_handle_container_start_with_label(
        ...
        with patch.object(monitor, "client") as mock_client:
            ...
            # DNS callback is called with HOSTIP, not container IP
            dns_callback.assert_called_once_with(
                "add", "test.example.com", "192.168.1.100"
            )

    def test_handle_container_start_without_label(
        self, monitor, dns_callback, mock_container_without_label
    ):
    ...
    def test_handle_container_stop_with_label(
        self, monitor, dns_callback, mock_container_with_label
    ):
    ...
    def test_handle_container_stop_without_label(
        self, monitor, dns_callback, mock_container_without_label
    ):
    ...
    def test_handle_container_event_no_container_id(self, monitor):
        ...
        with patch.object(
            monitor, "_handle_container_start"
        ) as mock_start, patch.object(monitor, "_handle_container_stop") as mock_stop:
    ...
    def test_get_container_hostname_with_label(
        self, monitor, mock_container_with_label
    ):
    ...
    def test_get_container_hostname_without_label(
        self, monitor, mock_container_without_label
    ):
    ...
    def test_process_existing_containers(self, monitor, dns_callback):
        ...
        mock_container2.attrs = {"Config": {"Labels": {}}}  # No joyride label
    ...
    def test_start_monitor(self, mock_docker_from_env, monitor, dns_callback):
        ...
        with patch.object(
            monitor, "_process_existing_containers"
        ) as mock_process, patch("threading.Thread") as mock_thread:
            monitor.start()
