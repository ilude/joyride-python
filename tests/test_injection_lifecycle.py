...
class MockComponent(JoyrideLifecycleComponent):
    ...
    def __init__(self, name: str, start_delay: float = 0.0, stop_delay: float = 0.0,
                 start_should_fail: bool = False, stop_should_fail: bool = False):
        super().__init__(name)
...
class MockService:
    ...
    def __init__(self, name: str, has_lifecycle: bool = False, has_health_check: bool = False):
        self.name = name
...
class TestJoyrideLifecycleComponent:
    ...
    def test_add_dependent(self):
        ...
        # Invalid dependent
        with pytest.raises(ValueError, match="Dependent name must be a non-empty string"):
...
class TestJoyrideLifecycleManager:
    ...
    def test_register_duplicate_component(self):
        ...
        with pytest.raises(ValueError, match="Component 'test_component' is already registered"):
    ...
    def test_register_invalid_component(self):
        ...
        with pytest.raises(ValueError, match="Component must be a JoyrideLifecycleComponent"):
    ...
    def test_add_circular_dependency(self):
        ...
        with pytest.raises(JoyrideLifecycleDependencyError, match="circular dependency"):
    ...
    async def test_start_component_failure(self):
        ...
        with pytest.raises(JoyrideLifecycleError, match="Component component1 failed to start"):
            await manager.start_all()
    ...
    async def test_stop_component_failure(self):
        ...
        with pytest.raises(JoyrideLifecycleError, match="Some components failed to stop"):
            await manager.stop_all()
...
class TestLifecycleIntegration:
    ...
    async def test_provider_component_integration(self):
        ...
        def create_service(registry):
            return MockService("test_service", has_lifecycle=True, has_health_check=True)
