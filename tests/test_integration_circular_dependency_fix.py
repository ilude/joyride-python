...
class TestCircularDependencyIntegration:
    ...
    def mock_docker_container(self):
        ...
        container.attrs = {
            ...
            "NetworkSettings": {
                "Networks": {"joyride_default": {"IPAddress": "192.168.16.191"}}
            },
    ...
    def test_full_integration_no_circular_dependency(
        self, mock_docker_container, mock_dns_server, mock_dns_sync_manager
    ):
        """
        ...
        def tracked_dns_record_callback(action, hostname, ip_address):
            ...
            if call_tracker["dns_callback_calls"] > 5:
                raise RecursionError(
                    f"Circular dependency detected! dns_record_callback called {call_tracker['dns_callback_calls']} times"
                )
        ...
        with patch(
            "app.main.dns_record_callback", side_effect=tracked_dns_record_callback
        ):
            ...
            assert (
                call_tracker["dns_callback_calls"] == 1
            ), f"Expected 1 callback call, got {call_tracker['dns_callback_calls']}"
            assert (
                call_tracker["sync_manager_calls"] == 1
            ), f"Expected 1 sync manager call, got {call_tracker['sync_manager_calls']}"

            # Verify DNS server was called
            mock_dns_server.add_record.assert_called_once_with(
                "joyride.example.com", "192.168.16.191"
            )

            # Verify DNS sync manager was called with local_only=True (the key fix)
            mock_dns_sync_manager.add_dns_record.assert_called_once_with(
                "joyride.example.com", "192.168.16.191", local_only=True
            )

    def test_docker_monitor_self_referential_container_processing(
    ...
    def test_concurrent_dns_operations_stability(self):
        ...
        def thread_safe_callback(action, hostname, ip_address):
            with lock:
                ...
                if call_tracker["total_calls"] > 50:  # Reasonable limit
                    call_tracker["errors"].append(
                        f"Too many callbacks: {call_tracker['total_calls']}"
                    )
        ...
        def worker_thread(thread_id):
            ...
            try:
                for i in range(5):
                    ...
                    sync_manager.add_dns_record(
                        f"normal-{thread_id}-{i}.test", f"10.0.{thread_id}.{i}"
                    )

                    # Local-only operation (should NOT trigger callback)
                    sync_manager.add_dns_record(
                        f"local-{thread_id}-{i}.test",
                        f"10.1.{thread_id}.{i}",
                        local_only=True,
                    )
        ...
        assert (
            call_tracker["total_calls"] == expected_callbacks
    ...
    def test_production_scenario_simulation(self, mock_docker_container):
        ...
        with patch("docker.from_env", return_value=mock_docker_client), patch(
            "app.main.dns_record_callback", side_effect=tracking_dns_callback
        ):
            ...
            dns_callback_count = len(
                [call for call in call_sequence if call.startswith("dns_callback_")]
            )
            assert (
                dns_callback_count == 1
            ), f"Expected 1 DNS callback, got {dns_callback_count}: {call_sequence}"
    ...
    def test_local_only_parameter_enforcement(self):
        ...
        for hostname, ip, local_only, should_trigger in test_cases:
            ...
            if should_trigger:
                ...
                ), f"Expected callback for {hostname} with local_only={local_only}"
                assert callback_calls[0] == ("add", hostname, ip)
            else:
                assert (
                    len(callback_calls) == 0
                ), f"Unexpected callback for {hostname} with local_only={local_only}"
        ...
        for hostname, ip, local_only, should_trigger in test_cases:
            ...
            if should_trigger:
                assert (
                    len(callback_calls) == 1
                ), f"Expected remove callback for {hostname} with local_only={local_only}"
                assert callback_calls[0] == ("remove", hostname, "")
            else:
                assert (
                    len(callback_calls) == 0
                ), f"Unexpected remove callback for {hostname} with local_only={local_only}"
    ...
    def test_regression_prevention_assertions(self):
        ...
        try:
            ...
            pytest.fail(
                f"local_only parameter missing from DNS sync manager methods: {e}"
            )
        ...
        try:
            ...
            pytest.fail(
                f"Backward compatibility broken for DNS sync manager methods: {e}"
            )
        ...
        try:
            ...
            with patch("app.main.dns_server"), patch(
                "app.main.dns_sync_manager"
                ...
                mock_sync_manager.add_dns_record.assert_called_with(
                    "regression-test.com", "1.2.3.4", local_only=True
                )

        except RecursionError:
            pytest.fail(
                "dns_record_callback is not using local_only=True, circular dependency detected!"
            )
        except Exception:
