...
class TestDockerEventProducer:
    ...
    def test_docker_producer_get_container_info(self, docker_producer):
        ...
        assert info["domainname"] == "example.com"
        assert info["dns_servers"] == ["8.8.8.8"]

    def test_docker_producer_get_container_info_not_found(self, docker_producer):
        """Test getting container info when container not found."""
        mock_client = Mock()
    ...
    def test_docker_producer_get_next_event_no_stream(self, docker_producer):
        """Test getting next event with no stream."""
        docker_producer._event_stream = None
        event = docker_producer._get_next_event()
        assert event is None

    def test_docker_producer_get_next_event_stop_iteration(self, docker_producer):
    ...
    def test_docker_producer_get_supported_event_types(self, docker_producer):
        ...
        assert "network.create" in event_types

    @pytest.mark.asyncio
    async def test_docker_producer_health_check(self, docker_producer):
        """Test Docker producer health check."""
    ...
    @pytest.mark.asyncio
    async def test_docker_producer_process_unknown_event(self, docker_producer):
        """Test processing unknown Docker event type."""
        event_data = {
            "Type": "unknown",
            "Action": "test",
            "Actor": {"ID": "test123"},
